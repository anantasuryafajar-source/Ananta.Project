...
const baris = (): Line => ({ product_id: "", description: "", quantity: "1", unit: "botol", unit_price: "0", discount: "0", tax_rate: "0" });
...
export default function SalesOrdersPage() {
  const [items, setItems] = useState<SO[] | null>(null);
  ...
  function muat() { api<SO[]>("/sales-orders").then(setItems).catch((e) => setError(e.message)); }
  useEffect(muat, []);

  function buka() {
    setFormError(null); setContactId(""); setDate(today()); setWhId(""); setCourier(""); setNotes(""); setLines([baris()]);
    setOpen(true);
  ...
  async function terbitkan(id: string) {
    setBusy(id);
    try { await api(`/sales-orders/${id}/invoice`, { method: "POST" }); muat(); }
    catch (err) { setError(err instanceof Error ? err.message : "Gagal menerbitkan faktur (cek stok)."); }
  ...
  return (
    <>
      ...
      <div className="p-6">
        ...
        {items?.length === 0 && (
          <Card className="text-center">
            <p className="text-ink">Belum ada sales order.</p>
            <p className="mt-1 text-sm text-ink-muted">Catat pesanan pelanggan; saat siap, klik “Terbitkan Faktur” untuk jadi Invoice + stok keluar.</p>
          </Card>
        )}
          ...
          <Card className="overflow-hidden p-0">
            <table className="w-full text-sm">
              <thead><tr className="border-b border-line text-left text-caption text-ink-muted">
                <th className="px-4 py-3 font-medium">No. SO</th>
                ...
                {items.map((v) => (
                  <tr key={v.id} className="border-b border-line last:border-0 hover:bg-surface-sunken">
                    ...
                    <td className="px-4 py-3 text-right">
                      {v.status === "draft" ? (
                        <Button variant="secondary" onClick={() => terbitkan(v.id)} disabled={busy === v.id}>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Buat Sales Order" width="max-w-3xl">
        <form onSubmit={simpan} className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 flex justify-end text-sm"><span className="mr-3 text-ink-muted">Total</span><span className="font-semibold tabular-nums text-ink">{rupiah(total)}</span></div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <Field label="Kurir (opsional)"><Input value={courier} onChange={(e) => setCourier(e.target.value)} placeholder="JNE / kurir internal" /></Field>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" disabled={saving}>{saving ? "Menyimpan…" : "Simpan SO"}</Button>
          </div>
        </form>
      </Modal>
    </>
