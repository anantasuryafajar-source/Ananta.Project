            ...
            if st.step == "confirm":
                if text.lower() in ("ya", "y", "iya"):
                    ...
                    try:
                        ...
                        await clear_state(db, chat_id)
                        await update.message.reply_text(
                            "Gagal menyimpan penjualan (kesalahan tak terduga)."
                        )
                        return
                    await db.refresh(inv)
                    await clear_state(db, chat_id)
                    await update.message.reply_text(
                        f"Penjualan tersimpan: {inv.number}\n"
                        f"Customer: {draft['customer_name']}\n"
                        f"Tagih via /bayar_customer saat uang masuk (Omzet Collect).\n"
                        "Cek di web Ananta -> menu Penjualan."
                    )
                else:
                    await update.message.reply_text("Ketik YA untuk simpan, atau /batal.")
...
async def _find_customer(db, company_id, name):
    q = name.strip().lower()
    rows = (
        await db.execute(
            select(Contact).where(
    ...
    if len(contains) == 1:
        return contains[0], None
    if not contains:
        return None, f"Customer '{name}' tidak ditemukan. Tambah via /tambah_kontak."
    names = ", ".join(c.name for c in contains[:5])
    return None, f"Beberapa customer cocok: {names}. Sebutkan lebih tepat."


async def cmd_jual(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.effective_chat.id
    full_text = update.message.text or ""
    parts = full_text.split(None, 1)
    body = parts[1].strip() if len(parts) > 1 else ""
    ...
    async with SessionLocal() as db:
        ...
        if u is None:
            ...
            return
        roles = await user_roles(db, u.id)
        if "owner" not in roles and not roles.intersection(JUAL_ROLES):
            await update.message.reply_text("Kamu tidak punya akses membuat penjualan.")
            return

        if not body:
            await update.message.reply_text(
                "Buat faktur penjualan (Omzet Lempar).\n\n" + JUAL_HINT
            )
            return

        parsed = parse_penjualan_block(body)
        if not parsed["customer"] or not parsed["items"]:
            await update.message.reply_text(
                "Customer dan minimal satu Item wajib.\n\n" + JUAL_HINT
            )
            return

        customer, err = await _find_customer(db, u.company_id, parsed["customer"])
        if err:
            await update.message.reply_text(err)
            return
    ...
    await update.message.reply_text(
        "Konfirmasi penjualan (Omzet Lempar):\n"
